...
static void print_usage() {
  fprintf(stderr, "Usage option 0: ./nrpyelliptic_conformally_flat_gpu [--help or -h] <- Outputs this usage command\n");
  fprintf(stderr, "Usage option 1: ./nrpyelliptic_conformally_flat_gpu <- reads in parameter file nrpyelliptic_conformally_flat_gpu.par\n");
  fprintf(stderr, "Usage option 2: ./nrpyelliptic_conformally_flat_gpu [parfile] <- reads in parameter file [parfile]\n");
  fprintf(stderr, "Usage option 3: ./nrpyelliptic_conformally_flat_gpu [convergence_factor] <- overwrites parameters in list after reading in "
                  "nrpyelliptic_conformally_flat_gpu.par\n");
  fprintf(stderr, "Usage option 4: ./nrpyelliptic_conformally_flat_gpu [parfile] [convergence_factor] <- overwrites list of steerable parameters "
...
void cmdline_input_and_parfile_parser(commondata_struct *restrict commondata, int argc, const char *argv[]) {
  ...
  if (argc == 1) {
    option = 1; // Usage option 1: Process default parameter file "nrpyelliptic_conformally_flat_gpu.par"
  } else if (argc == 2) {
    ...
    option = 3; // Usage option 3: Overwrite steerable parameters after processing "nrpyelliptic_conformally_flat_gpu.par"
  ...
  const char *filename = (option == 1 || option == 3) ? "nrpyelliptic_conformally_flat_gpu.par" : argv[1];
