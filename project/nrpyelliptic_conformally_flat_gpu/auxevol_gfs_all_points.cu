...
/*
 * GPU Kernel: auxevol_gfs_all_points_gpu.
 * GPU Kernel to initialize auxillary grid functions at all grid points.
 */
__global__ static void auxevol_gfs_all_points_gpu(const REAL *restrict x0, const REAL *restrict x1, const REAL *restrict x2, REAL *restrict in_gfs) {
  ...
  for (int i2 = tid2; i2 < Nxx_plus_2NGHOSTS2; i2 += stride2) {
    ...
    for (int i1 = tid1; i1 < Nxx_plus_2NGHOSTS1; i1 += stride1) {
      ...
      for (int i0 = tid0; i0 < Nxx_plus_2NGHOSTS0; i0 += stride0) {
        ...
        auxevol_gfs_single_point(xx0, xx1, xx2, &in_gfs[IDX4(PSI_BACKGROUNDGF, i0, i1, i2)], &in_gfs[IDX4(ADD_TIMES_AUUGF, i0, i1, i2)]);
...
#include "set_CodeParameters.h"
    ...
    auxevol_gfs_all_points_gpu<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(x0, x1, x2, in_gfs);
