...
/*
 * GPU Kernel: apply_bcs_outerextrap_and_inner_only_gpu.
 * GPU Kernel to apply extrapolation BCs to pure points.
 */
__global__ static void apply_bcs_outerextrap_and_inner_only_gpu(const int num_pure_outer_boundary_points, const int which_gz, const int dirn,
 ...
 */
static void apply_bcs_outerextrap_and_inner_only(const params_struct *restrict params, const bc_struct *restrict bcstruct, REAL *restrict gfs) {
    ...
    for (int dirn = 0; dirn < 3; dirn++) {
      if (bc_info->num_pure_outer_boundary_points[which_gz][dirn] > 0) {
        ...
        apply_bcs_outerextrap_and_inner_only_gpu<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(
            num_pure_outer_boundary_points, which_gz, dirn, pure_outer_bc_array, gfs);
