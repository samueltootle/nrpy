...
static void cpy_pure_outer_bc_array(bc_struct *restrict bcstruct_h, bc_struct *restrict bcstruct_d, const int idx, const int idx2d) {
  const int streamid = idx2d % nstreams;
  cudaMemcpyAsync(&bcstruct_d->pure_outer_bc_array[idx][idx2d].i0, &bcstruct_h->pure_outer_bc_array[idx][idx2d].i0, sizeof(short),
                  cudaMemcpyHostToDevice, streams[streamid]);
  cudaCheckErrors(cudaMemcpy, "Memcpy failed - pure_outer_bc_array1");
  cudaMemcpyAsync(&bcstruct_d->pure_outer_bc_array[idx][idx2d].i1, &bcstruct_h->pure_outer_bc_array[idx][idx2d].i1, sizeof(short),
                  cudaMemcpyHostToDevice, streams[streamid]);
  cudaCheckErrors(cudaMemcpy, "Memcpy failed - pure_outer_bc_array2");
  cudaMemcpyAsync(&bcstruct_d->pure_outer_bc_array[idx][idx2d].i2, &bcstruct_h->pure_outer_bc_array[idx][idx2d].i2, sizeof(short),
                  cudaMemcpyHostToDevice, streams[streamid]);
  cudaCheckErrors(cudaMemcpy, "Memcpy failed - pure_outer_bc_array2");
  cudaMemcpyAsync(&bcstruct_d->pure_outer_bc_array[idx][idx2d].FACEX0, &bcstruct_h->pure_outer_bc_array[idx][idx2d].FACEX0, sizeof(int8_t),
                  cudaMemcpyHostToDevice, streams[streamid]);
  cudaCheckErrors(cudaMemcpy, "Memcpy failed - pure_outer_bc_array Face0");
  cudaMemcpyAsync(&bcstruct_d->pure_outer_bc_array[idx][idx2d].FACEX1, &bcstruct_h->pure_outer_bc_array[idx][idx2d].FACEX1, sizeof(int8_t),
                  cudaMemcpyHostToDevice, streams[streamid]);
  cudaCheckErrors(cudaMemcpy, "Memcpy failed - pure_outer_bc_array Face1");
  cudaMemcpyAsync(&bcstruct_d->pure_outer_bc_array[idx][idx2d].FACEX2, &bcstruct_h->pure_outer_bc_array[idx][idx2d].FACEX2, sizeof(int8_t),
                  cudaMemcpyHostToDevice, streams[streamid]);
...
#include "../set_CodeParameters.h"
  ...
  /*
   *  Original SymPy expressions:
   *  "[Cart_to_xx0_inbounds = M_SQRT1_2*sqrt(Cartx**2 + Carty**2 + Cartz**2 - bScale**2 + sqrt(-4*Cartz**2*bScale**2 + bScale**4 +
   * 2*bScale**2*(Cartx**2 + Carty**2 + Cartz**2) + (Cartx**2 + Carty**2 + Cartz**2)**2))]"
   *  "[Cart_to_xx1_inbounds = acos(M_SQRT1_2*sqrt(1 + (Cartx**2 + Carty**2 + Cartz**2)/bScale**2 - sqrt(-4*Cartz**2*bScale**2 + bScale**4 +
   ...
   */
  {
    ...
    Cart_to_xx0_inbounds = M_SQRT1_2 * sqrt(-tmp1 + tmp2 + tmp3);
    Cart_to_xx1_inbounds = acos(M_SQRT1_2 * sqrt(tmp2 * tmp4 - tmp3 * tmp4 + 1) * (((Cartz) > 0) - ((Cartz) < 0)));
    Cart_to_xx2_inbounds = atan2(Carty, Cartx);
  }
...
#define EPS_REL 1e-8
  ...
  if (fabs((REAL)(xCart_from_xx - xCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
      fabs((REAL)(yCart_from_xx - yCart_from_xx_inbounds)) > EPS_REL * norm_factor ||
      fabs((REAL)(zCart_from_xx - zCart_from_xx_inbounds)) > EPS_REL * norm_factor) {
    fprintf(stderr,
            ...
            (REAL)xCart_from_xx, (REAL)yCart_from_xx, (REAL)zCart_from_xx, (REAL)xCart_from_xx_inbounds, (REAL)yCart_from_xx_inbounds,
            (REAL)zCart_from_xx_inbounds, xx[0][i0], xx[1][i1], xx[2][i2], xx[0][i0_inbounds], xx[1][i1_inbounds], xx[2][i2_inbounds],
...
#include "../set_CodeParameters.h"
  ...
  cudaMemcpyAsync(bcstruct_gpu->inner_bc_array, bcstruct->inner_bc_array, sizeof(innerpt_bc_struct) * bcstruct->bc_info.num_inner_boundary_points,
  ...
  for (int which_gz = 0; which_gz < NGHOSTS; which_gz++) {
    ...
    cudaMallocHost((void **)&bcstruct->pure_outer_bc_array[3 * which_gz + face / 2],
                   sizeof(outerpt_bc_struct) * 2 *
                       ((x0min_face_range[1] - x0min_face_range[0]) * (x0min_face_range[3] - x0min_face_range[2]) *
    ...
    cudaMallocHost((void **)&bcstruct->pure_outer_bc_array[3 * which_gz + face / 2],
                   sizeof(outerpt_bc_struct) * 2 *
                       ((x1min_face_range[1] - x1min_face_range[0]) * (x1min_face_range[3] - x1min_face_range[2]) *
    ...
    cudaMallocHost((void **)&bcstruct->pure_outer_bc_array[3 * which_gz + face / 2],
                   sizeof(outerpt_bc_struct) * 2 *
                       ((x2min_face_range[1] - x2min_face_range[0]) * (x2min_face_range[3] - x2min_face_range[2]) *
