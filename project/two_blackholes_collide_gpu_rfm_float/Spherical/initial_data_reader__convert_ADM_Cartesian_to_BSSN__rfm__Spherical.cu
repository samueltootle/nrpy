...
__device__ void ADM_Cart_to_BSSN_Cart(const commondata_struct *restrict commondata, const REAL xCart[3],
  ...
  const REAL tmp11 = (1.0F / 3.0F) * tmp10;
  ...
  BSSN_Cart_basis->cf = powf(tmp6 / (ADM_Cart_basis->gammaDD00 * ADM_Cart_basis->gammaDD11 * ADM_Cart_basis->gammaDD22 * tmp7 +
...
// #include "../set_CodeParameters.h"
  ...
  const REAL tmp0 = cosf(xx1);
  const REAL tmp2 = sinf(xx1);
  const REAL tmp4 = cosf(xx2);
  const REAL tmp7 = sinf(xx2);
  ...
  for(size_t i2 = tid2+NGHOSTS; i2 < Nxx2+NGHOSTS; i2 += stride2) {
    for(size_t i1 = tid1+NGHOSTS; i1 < Nxx1+NGHOSTS; i1 += stride1) {
      for(size_t i0 = tid0+NGHOSTS; i0 < Nxx0+NGHOSTS; i0 += stride0) {
         ...
         */
        const REAL FDPart3tmp0 = sinf(xx1);
        ...
        const REAL FDPart3tmp28 = cosf(xx1);
        ...
        const REAL FDPart3tmp19 = (1.0F / 2.0F) * FDPart3tmp18;
        ...
        const REAL FDPart3tmp22 = (1.0F / 2.0F) * FDPart3tmp21;
        ...
        const REAL FDPart3tmp62 = (1.0F / 2.0F) * FDPart3tmp50;
        const REAL FDPart3tmp64 = (1.0F / 2.0F) * FDPart3tmp60;
