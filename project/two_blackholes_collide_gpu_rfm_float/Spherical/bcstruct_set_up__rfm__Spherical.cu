...
#include "../set_CodeParameters.h"
  ...
  REAL xCart[3]; // where (x,y,z) is output
  {
     ...
     */
    {
      const REAL tmp0 = xx0 * sinf(xx1);
      xCart[0] = tmp0 * cosf(xx2);
      xCart[1] = tmp0 * sinf(xx2);
      xCart[2] = xx0 * cosf(xx1);
    }
  }
   ...
   */
  {
    ...
    Cart_to_xx1_inbounds = acosf(Cartz / tmp0);
    Cart_to_xx2_inbounds = atan2f(Carty, Cartx);
  }
  ...
  REAL xCart_from_xx, yCart_from_xx, zCart_from_xx;
  {
     ...
     */
    const REAL tmp0 = xx0 * sinf(xx1);
    xCart_from_xx = tmp0 * cosf(xx2);
    yCart_from_xx = tmp0 * sinf(xx2);
    zCart_from_xx = xx0 * cosf(xx1);
  }
  ...
  REAL xCart_from_xx_inbounds, yCart_from_xx_inbounds, zCart_from_xx_inbounds;
  {
     ...
     */
    const REAL tmp0 = xx0 * sinf(xx1);
    xCart_from_xx_inbounds = tmp0 * cosf(xx2);
    yCart_from_xx_inbounds = tmp0 * sinf(xx2);
    zCart_from_xx_inbounds = xx0 * cosf(xx1);
  }
...
#define EPS_REL 1e-6
  ...
  REAL REAL_parity_array[10];
  {
    ...
    */
     ...
     */
    {
      const REAL tmp0 = cosf(xx1) * cosf(xx1_inbounds);
      const REAL tmp1 = sinf(xx1) * sinf(xx1_inbounds);
      const REAL tmp2 = sinf(xx2) * sinf(xx2_inbounds);
      const REAL tmp3 = cosf(xx2) * cosf(xx2_inbounds);
 ...
 * including the ghostzone, with NGHOSTS=2.
