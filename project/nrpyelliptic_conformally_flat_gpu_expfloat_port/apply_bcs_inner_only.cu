...
/*
 * GPU Kernel: apply_bcs_inner_only_gpu.
 * GPU Kernel to apply extrapolation BCs to inner boundary points only.
 */
__global__ static void apply_bcs_inner_only_gpu(const int num_inner_boundary_points, const int which_gf,
...
void apply_bcs_inner_only(const commondata_struct *restrict commondata, const params_struct *restrict params, const bc_struct *restrict bcstruct,
  ...
  for (int which_gf = 0; which_gf < NUM_EVOL_GFS; which_gf++) {
    apply_bcs_inner_only_gpu<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(num_inner_boundary_points, which_gf, inner_bc_array, gfs);
