...
void params_struct_set_to_default(commondata_struct *restrict commondata, griddata_struct *restrict griddata) {
  ...
  for (int grid = 0; grid < commondata->NUMGRIDS; grid++) {
    ...
    params->Nxx0 = 256;                     // nrpy.infrastructures.BHaH.grid_management.cuda.numerical_grids_and_timestep::Nxx0
    params->Nxx1 = 256;                     // nrpy.infrastructures.BHaH.grid_management.cuda.numerical_grids_and_timestep::Nxx1
    ...
    params->grid_physical_size = 1000000.0; // nrpy.reference_metric::grid_physical_size
  }
}
