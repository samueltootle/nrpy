...
__global__ static void compare(const float *restrict auxgfs_exp, const float *restrict ingfs_exp, const float *restrict rhsgfs_exp, 
  ...
  if(i < Ntot) {
    ...
    if(std::fabs(ref) > 0) {
      ...
      if(rel > 1e-13) {
        printf("ausgfs failure at %ul; %1.15e, %1.15e, %1.15e\n", i, ref, cmp, rel);
      }
    }
...
__global__ static void decompose_gf(const REAL *restrict gf_in, float *restrict gf_out) {
  const int Nxx_plus_2NGHOSTS0 = d_params.Nxx_plus_2NGHOSTS0;
  const int Nxx_plus_2NGHOSTS1 = d_params.Nxx_plus_2NGHOSTS1;
  const int Nxx_plus_2NGHOSTS2 = d_params.Nxx_plus_2NGHOSTS2;
  const int Ntot = Nxx_plus_2NGHOSTS0 * Nxx_plus_2NGHOSTS1 * Nxx_plus_2NGHOSTS2 * NUM_EVOL_GFS;
...
__host__ static void setup_expansion_gfs( const params_struct *restrict params, 
  ...
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(auxevol_gfs, auxgfs_exp);
  cudaCheckErrors(cudaKernel, "decompose auxgf failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(in_gfs, ingfs_exp);
...
__host__ static void setup_expansion_rfm(const params_struct *restrict params, 
  ...
  // Decompose REAL into floats and store in the new arrays
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f0_of_xx0, exp_f0_of_xx0);
  cudaCheckErrors(cudaKernel, "decompose rfm_f0_of_xx0 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f0_of_xx0__D0, exp_f0_of_xx0__D0);
  cudaCheckErrors(cudaKernel, "decompose rfm_f0_of_xx0__D0 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f0_of_xx0__DD00, exp_f0_of_xx0__DD00);
  cudaCheckErrors(cudaKernel, "decompose rfm_f0_of_xx0__DD00 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f0_of_xx0__DDD000, exp_f0_of_xx0__DDD000);
  cudaCheckErrors(cudaKernel, "decompose rfm_f0_of_xx0__DDD000 failure");

  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f1_of_xx1, exp_f1_of_xx1);
  cudaCheckErrors(cudaKernel, "decompose rfm_f1_of_xx1 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f1_of_xx1__D1, exp_f1_of_xx1__D1);
  cudaCheckErrors(cudaKernel, "decompose rfm_f1_of_xx1__D1 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f1_of_xx1__DD11, exp_f1_of_xx1__DD11);
  cudaCheckErrors(cudaKernel, "decompose rfm_f1_of_xx1__DD11 failure");

  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f2_of_xx0, exp_f2_of_xx0);
  cudaCheckErrors(cudaKernel, "decompose rfm_f2_of_xx0 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f2_of_xx0__D0, exp_f2_of_xx0__D0);
  cudaCheckErrors(cudaKernel, "decompose rfm_f2_of_xx0__D0 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f2_of_xx0__DD00, exp_f2_of_xx0__DD00);
  cudaCheckErrors(cudaKernel, "decompose rfm_f2_of_xx0__DD00 failure");

  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f4_of_xx1, exp_f4_of_xx1);
  cudaCheckErrors(cudaKernel, "decompose rfm_f4_of_xx1 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f4_of_xx1__D1, exp_f4_of_xx1__D1);
  cudaCheckErrors(cudaKernel, "decompose rfm_f4_of_xx1__D1 failure");
  decompose_gf<<<blocks_per_grid, threads_per_block, sm, streams[streamid]>>>(rfm_f4_of_xx1__DD11, exp_f4_of_xx1__DD11);
...
#include "set_CodeParameters.h"
  ...
  float* ingfs_exp;
  float* auxgfs_exp;
  cudaMalloc(&auxgfs_exp, sizeof(float) * NUM_EVOL_GFS * Ntot * 2U);
